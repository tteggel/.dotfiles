...
commands=(
  ...
  "system:Rebuild system:if command -v nixos-rebuild >/dev/null 2>&1; then sudo nixos-rebuild switch --flake ~/src/github.com/tteggel/.dotfiles#thixos; else home-manager switch --flake ~/src/github.com/tteggel/.dotfiles#thom@nix; fi	[Sys] Rebuild system"
  "system:Update system:git -C ~/src/github.com/tteggel/.dotfiles diff --quiet flake.lock || { echo 'Error: flake.lock has uncommitted changes'; exit 1; } && nix flake update --flake ~/src/github.com/tteggel/.dotfiles && if command -v nixos-rebuild >/dev/null 2>&1; then sudo nixos-rebuild switch --flake ~/src/github.com/tteggel/.dotfiles#thixos; else home-manager switch --flake ~/src/github.com/tteggel/.dotfiles#thom@nix; fi && git -C ~/src/github.com/tteggel/.dotfiles add flake.lock && git -C ~/src/github.com/tteggel/.dotfiles commit -m 'Update flake inputs' && git -C ~/src/github.com/tteggel/.dotfiles push	[Sys] Update system"
)
